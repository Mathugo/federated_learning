...
class Task:
    ...
    @staticmethod
    def createTask(loss_fn, val_fn, deeplab_model, num_classes, aggregation_function=Median()):
        TI = TaskInterface()
        ...
        @TI.add_kwargs(**{'loss_fn': loss_fn, 'num_classes': num_classes, 'deeplab_model': deeplab_model})
        @TI.register_fl_task(model='model', data_loader='train_loader', \
                            device='device', optimizer='optimizer')     
        @TI.set_aggregation_function(aggregation_function)
        def train(model, train_loader, optimizer, device, loss_fn, num_classes, deeplab_model):
        ...
        def validate(model, val_loader, device, val_fn, loss_fn, num_classes):
            ...
            return {'val_score ({})'.format(val_score): val_score / total_samples, 'val_loss ({})'.format(str(loss_fn)): np.mean(losses)}
